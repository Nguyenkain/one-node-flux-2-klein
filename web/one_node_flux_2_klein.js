...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType,nodeData){
    ...
    nodeType.prototype._buildUI=function(){
      ...
      {
        const _initKey=S.pill==="edit"?"promptEdit":S.pill==="inpaint"?"promptPaint":S.pill==="faceswap"?"promptFs":S.pill==="pose"?"promptPose":S.pill==="i2i"?"promptI2i":S.pill==="llm"?"promptLLM":"promptT2i";
      ...
      _loadSeedVR2Models();
      _loadUpscaleModels();
      // modGrid2 = Flux-only loaders (faceswap LoRA + pose LoRA + remove-bg). Hidden in Krea.
      modGrid2.append(fsLoraF.wrap,poseLoraF.wrap,bgF.wrap);
      ...
      topBarLeft.append(pillT2I,pillI2I,pillEdit,pillInpaint,pillFaceswap,pillPose,pillUpscale,pillLLM);
      ...
      const _pillPromptKey=(p)=>{
        ...
        return p==="t2i"?"promptT2i":p==="edit"?"promptEdit":p==="inpaint"?"promptPaint":p==="i2i"?"promptI2i":p==="pose"?"promptPose":"promptFs";
      };
      ...
      function setPill(p){
        ...
        persist();
        [pillT2I,pillI2I,pillEdit,pillInpaint,pillFaceswap,pillPose,pillUpscale,pillLLM].forEach(b=>{
          const isActive=
            (b===pillT2I&&p==="t2i")||
            (b===pillI2I&&p==="i2i")||
            (b===pillEdit&&p==="edit")||
            ...
            (b===pillLLM&&p==="llm");
          b.style.display=(b===pillEdit||b===pillInpaint||b===pillFaceswap||b===pillPose||b===pillUpscale)?(S.modelFamily==="flux"?"":"none"):"";
      ...
      const _initNodeFsOverlay=()=>{
        ...
        ov._open=(type,src,name,opts)=>{
          // Clean up any previous comparer/image drag listeners + Use-as button before
...
width:"34px",background:C.bg2,border:`1px solid ${C.border}`,borderRadius:"4px",
      ...
      const _comparerBeforeUrl=(snapMode,snapImg1,beforeUrlOverride)=>{
        // Caller-supplied before URL (DWPose skeleton, or seedvr2/ultrasharp OUTPUT source).
        if(beforeUrlOverride) return beforeUrlOverride;
      ...
      const _mkULRow=(idx)=>{
        ...
        const _applyRowState=()=>{
          const disabled=!!S.userLoras[idx]?.disabled;
          row.style.opacity=disabled?"0.55":"1";
          ulDD.el.style.opacity=disabled?"0.7":"1";
          ulStr.style.opacity=disabled?"0.65":"1";
          trigRow.style.opacity=disabled?"0.65":"1";
        };
        ...
        const ulDD=DD(["none"],"none",v=>{
          ...
          if(!has){
            S.userLoras[idx].strength=0; S.userLoras[idx].disabled=false; ulStr.value="0";
          } else {
        ...
        const ulEnabledWrap=mk("label",{display:"inline-flex",alignItems:"center",gap:"4px",cursor:"pointer",flexShrink:"0",userSelect:"none"});
        const ulEnabled=mk("input",{accentColor:LIME,margin:"0"},{type:"checkbox"});
        ulEnabled.checked=!S.userLoras[idx].disabled;
        const ulEnabledTxt=mk("span",{fontSize:"8px",color:C.muted,fontWeight:"700",letterSpacing:".04em"});
        tx(ulEnabledTxt,"ON");
        ulEnabled.onchange=()=>{
          S.userLoras[idx].disabled=!ulEnabled.checked;
          _applyRowState();
          _ulUpdateBtn();persist();
        };
        ulEnabledWrap.append(ulEnabled,ulEnabledTxt);
        ...
        ulStr.onblur=()=>{ const p=_pf(ulStr.value); S.userLoras[idx].strength=isNaN(p)?1:p;
          ulStr.value=String(S.userLoras[idx].strength);_ulUpdateBtn();persist(); };
        ulStr.oninput=()=>{ S.userLoras[idx].strength=_pf(ulStr.value)||0;_ulUpdateBtn();persist(); };
        ...
        (()=>{
          ...
          ulStr.addEventListener("pointermove",(e)=>{
            ...
            S.userLoras[idx].strength=v; _ulUpdateBtn(); persist();
          });
        ...
        tx(ulDel,"DEL");
        ulDel.onmouseenter=()=>{ ulDel.style.background="rgba(255,80,80,.14)";ulDel.style.borderColor="rgba(255,80,80,.3)";ulDel.style.color="#ff7777"; };
        ulDel.onmouseleave=()=>{ ulDel.style.background="rgba(255,255,255,.05)";ulDel.style.borderColor="rgba(255,255,255,.12)";ulDel.style.color=C.muted; };
        ulDel.onclick=()=>{
          if(S.userLoras.length>_UL_DEFAULT){
      ...
      while(S.userLoras.length<_UL_DEFAULT) S.userLoras.push(_emptyUserLora());
      ...
      const _ulAddSlot=()=>{
        ...
        S.userLoras.push(_emptyUserLora());
        _ulRebuildRows();
        _ulSyncSlotsToModels();
        persist();
      };
      ...
      const _buildPromptWithTriggers=async(basePrompt)=>{
        ...
        for(const ul of S.userLoras){
          if(!_isActiveUserLora(ul)) continue;
      ...
      const _ulUpdateBtn=()=>{
        const n=S.userLoras.filter(_isActiveUserLora).length;
...
${base}`;
      ...
      const showFinal=(url,filename,subfolder)=>{
        ...
        // Local modes whose "before" is an OUTPUT image (seedvr2/ultrasharp) or a URL
        // (pose skeleton) — resolve to an explicit before-URL override for _applyComparer.
        const _seedvr2Source=S._pendingMeta?.seedvr2Source||null;
        const _ultrasharpSource=S._pendingMeta?.ultrasharpSource||null;
        let _beforeOverride=null;
        if(_snapMode==="pose"){
          _beforeOverride=_poseSkeletonUrl; // before = DWPose skeleton
        } else if(_snapMode==="seedvr2"&&_seedvr2Source?.filename){
          _beforeOverride=api.apiURL(`/view?filename=${encodeURIComponent(_seedvr2Source.filename)}&type=output&subfolder=${encodeURIComponent(_seedvr2Source.subfolder||"")}`);
        } else if(_snapMode==="ultrasharp"&&_ultrasharpSource?.filename){
          _beforeOverride=api.apiURL(`/view?filename=${encodeURIComponent(_ultrasharpSource.filename)}&type=output&subfolder=${encodeURIComponent(_ultrasharpSource.subfolder||"")}`);
        }
        if(!_applyComparer(url,_snapMode,_snapImg1,_beforeOverride)){
      ...
      genBtn.onclick=async()=>{
        ...
        if(!S.prompt.trim()&&!_kreaBuilderActive&&activePill!=="faceswap"&&activePill!=="llm"&&activePill!=="pose"){showError("Please enter a prompt.");return;}
        if(activePill==="llm"){
        ...
        if(activePill!=="upscale"&&!_activeModel&&!_hasExtModel&&activePill!=="llm"){showError("No model selected. Open Settings and choose a model.");return;}
        ...
        // Snapshot batch size used for this submission.
